...
var RingLib = require('./ring'),
  Ring = RingLib.Ring,
  Vertex = RingLib.Vertex;
...
function processPolygons(list, subject, clipper, sPoly, cPoly, s_forward, c_forward) {
  ...
  // Union Mode. If the list is empty, the polys didn't intersect
...
function setEntryExit(list, list2, s_forward, c_forward, sPoly, cPoly) {
  var curr = list.first;
  var sInside = turfInside(turfPoint([list.first.x, list.first.y]), cPoly);
  var cInside = turfInside(turfPoint([list2.first.x, list2.first.y]), sPoly);
  var sForward = !!(sInside ^ s_forward);
  var cForward = !!(cInside ^ c_forward);
